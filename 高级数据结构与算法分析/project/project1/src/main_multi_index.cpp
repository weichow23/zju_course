...
int main(){
    ...
    load_index();                                               //load the map to the disk
    return 0;
}
int examine(ifstream& in, const string& file){
    string buf;
    ...
    while(getline(in,buf)){
        for(int i=0;i<buf.length();i++)
        {
            ...
            {                                                       //Got a non-alphabetic character
...
void load_index(){                                                  //store the elements according to the first alpha
    ofstream out;                               
    string start;
    string tmp="abcdefghijklmnopqrstuvwxyz";                        //because the load is according to the alpha
    char last='a';
    int cnt=0;
    out.open("./SearchKey/a.txt",ios::out);
    for(auto &i:indexs){                                            //iterator the map, load every elemnet
        if(i.first[0]!=last){
            last=i.first[0];
            cnt++;
            out.close();
            out.open("./SearchKey/"+tmp.substr(cnt,1)+".txt",ios::out);
        }
