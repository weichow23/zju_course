...
- C++动态内存分配
  ...
  - delete用于删除动态分配的内存
    ...
    - 两个指针p1,p2指向同一个数据，如果p1被delete了，p2也不能访问原本p1指向的变量的值，因为**delete删除的是内存里的数据**（==相当于清空内存，并且收回==）, 而不是变量名
...
- 给**变量**(引用的对象**不能**是表达式)取了一个别名，使用方法为`type &refname = name;` 
- 引用和指针的区别
...
- 在初始化之后就不能改变其值，const型变量不能在连接单元外使用
...
- Destructor 析构函数
  ...
  类名前面加一个~，析构函数不需要参数，在类的生命周期结束的时候会**被编译器自动调用** 
...
- copy constructor拷贝构造函数
  ...
  class A {
    A() {}  
    A(const A& a) {} // 拷贝构造函数，一般为class (const class& copy_var)
